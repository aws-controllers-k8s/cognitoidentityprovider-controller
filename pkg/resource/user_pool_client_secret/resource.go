...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	f0, f0ok := identifier.AdditionalKeys["userPoolClientID"]
	if f0ok {
		r.ko.Spec.UserPoolClientID = aws.String(f0)
	}
	f2, f2ok := identifier.AdditionalKeys["userPoolID"]
	if f2ok {
		r.ko.Spec.UserPoolID = aws.String(f2)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	...
	f0, ok := fields["userPoolClientID"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: userPoolClientID"))
	}
	r.ko.Spec.UserPoolClientID = &f0
	f2, ok := fields["userPoolID"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: userPoolID"))
	}
	r.ko.Spec.UserPoolID = &f2
